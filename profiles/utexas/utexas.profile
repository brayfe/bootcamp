...
function install_utexas_welcome($form, &$form_state, &$install_state) {
  ...
  $message .= '<p>' . st('It packages a custom login module which allows authentication via UTLogin, the centralized authentication service. This module requires configuration of a Web Policy Agent. For installation instructions, see ') . l(t('the ITS Website'), 'http://www.utexas.edu/its/help/ut-drupal-kit/2489') . '.</p>';
